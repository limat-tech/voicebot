...
// --- FIX IS HERE ---
// Updated the type to match the API response you provided
type CartItem = {
    cart_item_id: number; // Changed from item_id
    product_id: number;
    name_en: string;
    price_per_unit: number; // Changed from price
    quantity: number;
    subtotal: number;
    image_url?: string;
    unit_type?: string;
...
const CartScreen = ({ navigation }: CartScreenProps) => {
    ...
    const handleRemoveItem = async (cartItemId: number) => {
        try {
            const token = await AsyncStorage.getItem('userToken');
            // Your backend DELETE endpoint for removing items likely expects the cart_item_id
    ...
    const renderItem = ({ item }: { item: CartItem }) => (
        <View style={styles.itemContainer}>
            ...
            <TouchableOpacity onPress={() => handleRemoveItem(item.cart_item_id)} style={styles.removeButton}>
    ...
    return (
        <View style={styles.container}>
            <FlatList
                ...
                // --- FIX IS HERE ---
                keyExtractor={(item) => item.cart_item_id.toString()} // Changed from item.item_id
...
const styles = StyleSheet.create({
    ...
    itemContainer: { backgroundColor: '#fff', padding: 15, marginVertical: 8, marginHorizontal: 10, borderRadius: 8, elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.1, shadowRadius: 2 },
    itemName: { fontSize: 17, fontWeight: 'bold', color: '#333', flexShrink: 1 },
    itemInfoRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 10 },
    itemQuantity: { fontSize: 15, color: '#666' },
    itemPrice: { fontSize: 16, fontWeight: '600', color: '#000' },
    itemDetailsRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 15 },
    quantityContainer: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#f0f0f0', borderRadius: 5 },
    quantityButton: { paddingHorizontal: 15, paddingVertical: 5 },
    quantityButtonText: { fontSize: 20, fontWeight: 'bold', color: '#007AFF' },
    quantityText: { fontSize: 16, fontWeight: '600', paddingHorizontal: 10 },
    removeButton: { alignSelf: 'flex-end', marginTop: 10, padding: 5 },
    removeButtonText: { color: 'red', fontSize: 14 },
    footer: { marginTop: 20, padding: 15, borderTopWidth: 1, borderColor: '#e0e0e0', backgroundColor: '#fff' },
